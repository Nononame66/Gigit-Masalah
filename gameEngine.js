/* ==========================================================
   Gigit Masalah (Fish It Style) - Main Game Engine
...
export class GameEngine {
  ...
  startReeling() {
    ...
    const difficulty = rarityDifficulty[this.caughtFish.rarity] || 0.75;
    ...
    this.targetZoneVel = 0.28 * difficulty;
  ...
  rollFish() {
    ...
    const lootTable = [
      { type: 'junk', id: 'kayu', name: 'Kayu Hanyut', symbol: '🪵', chance: 30, price: 2, xpReward: 1 },
      { type: 'junk', id: 'batu', name: 'Batu Sungai', symbol: '🪨', chance: 22, price: 1, xpReward: 1 },
      { type: 'junk', id: 'botol', name: 'Botol Plastik', symbol: '🧴', chance: 15, price: 3, xpReward: 1 },
      { type: 'junk', id: 'sendal', name: 'Sendal Sebelah', symbol: '🩴', chance: 12, price: 5, xpReward: 2 },
      { type: 'junk', id: 'kaleng', name: 'Kaleng Bekas', symbol: '🥫', chance: 9, price: 4, xpReward: 2 },
  ...
  updateReelingMinigame(delta) {
    ...
    if (isPressing) {
      ...
      this.playerBarVel -= 105 * delta;
    }

    this.playerBarVel *= 0.91;
    ...
    // Wide target zone so the mechanic is readable on phones.
    const targetHeight = 20 + (this.currentControl / 3); // Was 18 + control/4
    const isInside = (
    ...
    if (isInside) {
      ...
      const progressSpeed = 28 + (this.currentControl / 5);
      this.reelProgress += progressSpeed * delta;
      
      // Tension decreases VERY SLOWLY
      const tensionDecay = 24 + (this.currentResilience / 4);
      this.reelTension = Math.max(0, this.reelTension - tensionDecay * delta);
    } else {
      // Outside zone - progress decreases MUCH FASTER
      this.reelProgress = Math.max(0, this.reelProgress - 10 * delta);
      
      // Tension increases EXTREMELY FAST
      const tensionRate = (16 + this.fishDifficulty * 10) * (100 / (this.currentResilience + 60));
