/* ==========================================================
   Gigitan Kadal (Fish It Style) - DOM UI & Hotbar Controller
   ========================================================== */

import * as THREE from 'three';
import { storage } from './storage.js';
import { audio } from './audio.js';
import { FISH_SPECIES, createVoxelFishModel } from './voxelModels.js';
...
export class UIManager {
  ...
  showBiteAlert() {
    this.statusMessage.textContent = 'GIGITAN KADAL! KLIK SEKARANG!';
  ...
  showCatchModal(fish) {
    ...
    this.renderFish3DPreview(fish.id);
    modal.classList.remove('hidden');
  }

  renderFish3DPreview(fishId) {
    ...
    const fishModel = createVoxelFishModel(fishId);
    scene.add(fishModel);

    let animId;
    const animate = () => {
      animId = requestAnimationFrame(animate);
      fishModel.rotation.y += 0.02;
      fishModel.rotation.x = Math.sin(Date.now() * 0.002) * 0.15;
  ...
  openFishdex() {
    ...
    FISH_SPECIES.forEach(species => {
      const entry = storage.state.fishdex[species.id];
      const isUnlocked = entry && entry.unlocked;

      if (isUnlocked) {
        unlockedCount++;
        if (entry.maxWeight > maxOverallWeight) maxOverallWeight = entry.maxWeight;
      }
      ...
      card.innerHTML = `
        ...
        <div class="fish-rarity" style="background-color:${species.rarityColor}">${species.rarity}</div>
        <div style="font-size:0.7rem; color:#cbd5e1; margin-top:4px;">
          ${isUnlocked ? `Tertangkap: ${entry.count}x<br>Rekor: ${entry.maxWeight} kg` : 'Belum Ditemukan'}
        </div>
      `;
      grid.appendChild(card);
    });

    document.getElementById('fishdex-count').textContent = `${unlockedCount}/${FISH_SPECIES.length}`;
    document.getElementById('fishdex-max-weight').textContent = `${maxOverallWeight} kg`;
